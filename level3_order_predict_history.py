...
"""
Update order forecast result of Level-3.
...
def update_history_for_level3_order(level3_order_data: Level3OrderDataLoader,
                                    model_config: Bunch,
                                    db_config: Bunch,
                                    start_pred_year: int,
    ...
    """Update order forecast result of level3 in specified month."""
    ...
    X_train, y_train = level3_order_data.prepare_training_set(train_months, gap=gap)
    X_test = level3_order_data.prepare_testing_set(start_pred_year, start_pred_month, gap=gap)
    ...
    df_test = level3_order_data.get_true_data(true_pred_year, true_pred_month)
    df_pred_test = level3_order_data.decorate_pred_result(preds_test,
                                                          true_pred_year,
                                                          true_pred_month,
    ...
    customer_info_dict = level3_order_data.customer_info.to_dict()
    ...
    sku_info_dict = level3_order_data.sku_info.to_dict()
...
if __name__ == '__main__':
    ...
    # Update forecast result of level3 order
    # ============================================================================================ #
    ...
    for ym_str in pred_months:
        ...
        if date(start_pred_year, start_pred_month, 1) <= date(year_upper_bound, month_upper_bound, 1):
            update_history_for_level3_order(level3_order_data=level3_order_data,
